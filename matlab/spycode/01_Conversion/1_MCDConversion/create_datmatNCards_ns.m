function [execMessage, success] = create_datmatNCards_ns(idCh, output, startFolder,...
    ...
    filtConversionFlag, nMeas,filesToConvertNum,nummcd,spksflag)
...
for i = 1 : length(fnlUnFllId)
    ...
    switch dataId
        ...
        otherwise
            ...
            %Different type of Result folder if there are spike files...AA
            if idxCrrData==6
               
             currFold = fullfile(strtrim(char(outFolders{1}(amplIdx,:))), [expNum,'_PeakDetectionMAT'], strtrim(['ptrain_', char(expNames{1}(amplIdx,:))])); 
            else
...
for chan = 1 : length(EntityInfo)
    ...
    %waitbarImproved(chan/(length(EntityInfo)), ['converting stream ' num2str(chan) ' of '   num2str(length(EntityInfo))]);
     if ~exist('hWait','var')
        hWait = waitbar(chan/(length(EntityInfo)), ['converting stream ' num2str(chan) ' of '   num2str(length(EntityInfo))]);
    else
        waitbar(chan/(length(EntityInfo)), hWait, ['converting stream ' num2str(chan) ' of '   num2str(length(EntityInfo))]);
    end
    ...
    switch EntityInfo(chan).EntityType
        case 1
            ...
            % save data
            [spksflag]=writeDataToFile(crrData2Save, expNames, fnlUnFllId, crtdFld, EntityInfo(chan).EntityLabel(1:8), ch,nMeas,spksflag);
        ...
        case 3
            ...
            % variable for saving data #sparse and transpose...AA 
            crrData2Save = sparse(zeros(1,TimeSpanSamples)');
...
ns_CloseFile(hfile);
delete (hWait);
% if spksflag==1 && filesToConvertNum==nummcd
%     msgbox({'Please remember' 'artifact_detection=[]' 'consider to use replace_artifact function'}, 'Warn', 'Warn')
%     return
% end
...
%--------------------------------------------------------------------------
function [spksflag]=writeDataToFile(data, expNames, fnlUnFllId, crtdFld, crrEntityType, ch,nMeas,spksflag)
%--------------------------------------------------------------------------
...
for i = 1 : size(foldToSaveIn, 1)
    if nMeas==2
        if mod(idx,2)==0
            fileName = strtrim(char(expNames{1}(2, :)));
        else
            fileName = strtrim(char(expNames{1}(1, :)));
        end
    ...
    if crrEntityType(1:4)=='spks'
    filename_mat = fullfile(foldToSaveIn(i, :), ['ptrain_' fileName '_' ch '.mat']);
    peak_train=data;
     artifact = [];
     spikes = [];
    save(filename_mat,'peak_train','artifact','spikes');
    clear peak_train
    spksflag=1;
    else
